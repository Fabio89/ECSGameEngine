...
[[nodiscard]]
JsonDocument Json::fromFile(const std::filesystem::path& path)
{
    JsonDocument doc;
    if (std::ifstream ifs{path}; check(!ifs.bad(), std::format("Failed to open file: '{}'", path.generic_string())))
    {
...
void Json::toFile(const JsonDocument& document, const std::filesystem::path& path)
{
    std::ofstream ofs{path};
    if (!ofs.is_open())
    {
        report(std::format("Failed to open file: '{}'", path.generic_string()));
        return;
    }
